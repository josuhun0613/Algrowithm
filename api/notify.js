...
async function handleTelegram(req, res) {
    try {
        ...
        if (type === 'seminar') {
            ...
            const { name, phone, age, status, org, major, resultType, totalScore, selectedPrize, ref } = req.body;
            if (!name || !phone) return res.status(400).json({ error: '이름과 연락처는 필수입니다.' });
            telegramMessage = `📊 *AI 리서치 응모*\n\n📅 *접수 시간:* ${now}\n\n👤 *이름:* ${name}\n🎂 *나이:* ${age || '미입력'}세\n📱 *연락처:* ${phone}\n💼 *현재 상태:* ${status || '미선택'}\n🏫 *소속:* ${org || '미입력'}\n📚 *전공/관심:* ${major || '미입력'}\n\n🧠 *유형:* ${resultType || '-'} (${totalScore || 0}점)\n🎁 *희망 경품:* ${selectedPrize || '미선택'}\n📣 *유입 경로:* ${ref || 'direct'}\n\n---\n_AI 리서치 페이지에서 접수됨_`;
        } else {
